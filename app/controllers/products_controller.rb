class ProductsController < ApplicationController
  ...
  def index
    ...
    if params[:sub]
      @products = Product.includes(:product_infos).where("product_category_id = #{@sub_category.id}").paginate(:page => params[:page], :per_page => 2)
    else
      @products = Product.includes(:product_infos).where(product_category_id: select_ids).paginate(:page => params[:page], :per_page => 2)
    end
  end
